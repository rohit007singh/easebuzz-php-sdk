...
class Easebuzz{
    ...
    public function __construct($merchant_key, $salt, $env)
    {
    ...
    */
    ...
    {
        $ApiResponse = $this->initiate_payment($params, $redirect);
        return $ApiResponse;
    }
    ...
    *
    */
    function initiate_payment($params, $redirect)
    {
        $result = $this->payment($params, $redirect);

        return $result;
        // if ($redirect) {
        //     return $this->paymentResponse((object) $result);
        // } else {
        //     if($result->status==1){
    ...
    *
    */
    function payment($params, $redirect)
    {
        $postedArray = '';
        $URL = '';

        // argument validation
        $argument_validation = $this->checkArgumentValidation($params, $this->merchant_key, $this->salt, $this->env);
        ...
        $postedArray = $this->removeSpaceAndPreparePostArray($params);

        // empty validation
        $empty_validation = $this->emptyValidation($postedArray);
        ...
        // type validation
        $type_validation = $this->typeValidation($postedArray);
        ...
        // email validation
        $email_validation = $this->email_validation($postedArray['email']);
        if ($email_validation !== true)
            return $email_validation;

        // get URL based on enviroment like ($env = 'test' or $env = 'prod')
        $URL = $this->getURL($this->env);

        // process to start pay
        $pay_result = $this->pay($postedArray, $redirect, $this->salt, $URL);
    ...
    /*
    * $this-> method show response after API call.
    *
    ...
    *
    */
    function paymentResponse($result)
    {
    ...
    *  
    */
    function checkArgumentValidation($params, $merchant_key, $salt, $env)
    {
        $args = func_get_args();
        $argsc = count($args);
        if ($argsc !== 4) {
    ...
    *
    */
    function removeSpaceAndPreparePostArray($params)
    {
        $temp_array = array();
...
/*
    ...
    */
    function response($response_params, $salt_key){
        ...
        $easebuzzPaymentResponse = $this->removeSpaceAndPrepareAPIResponseArray($response_params);

        // empty validation 
        $empty_validation = $this->emptyValidation($easebuzzPaymentResponse);
        if (is_array($empty_validation) && $empty_validation['status'] === 0) {
            return $empty_validation;
        }
        ...
        $response_result = $this->getResponse($easebuzzPaymentResponse, $salt_key);
